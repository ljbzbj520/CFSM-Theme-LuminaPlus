...
- **延迟探测**：CF-Server-Monitor 的探测点固定为电信 / 联通 / 移动 / BD 四条线路，
  对应主题里的四条「线路」。默认开启多线路模式（默认三条：电信 / 联通 / 移动），**条数可自定**
  （v1.2.14 起，设置页里 1~4 条随便选，不再固定三条）；关掉后是单线路模式，
  可为每个节点单独指定线路，未指定的显示电信。探测目标与探测方式（ICMP / TCP）都在后台的
  服务器编辑里配置，公开接口不下发，主题读不到也无法展示。

  **线路的显示名跟着后端走**（v1.2.13 起）：后台给四条线路起的名字会由 `/api/config` 下发
  （`custom_ct_name` / `custom_cu_name` / `custom_cm_name` / `custom_bd_name`），主题在首页
  卡片、详情页 Ping 图例和设置页里一起用它。某一条没设就单独用主题的默认名，四条都没设
  （或后端版本较旧不下发）就完全维持电信 / 联通 / 移动 / BD。线路的 id 与探测目标仍由后端
  固定，可变的只有名字。
